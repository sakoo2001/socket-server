...
function nextTurn(mid) {
  ...
  // your_turn göndər — spinnerPid + targetPid birlikdə
  ...
  console.log(`[${mid}] your_turn → spinner=${spinnerId} target=${targetId}`);

  // 12s içərisində spin gəlməzsə auto_spin, 5s sonra növbəti tur
  ts.timer = setTimeout(() => {
    if (getTurn(mid).current !== spinnerId) return;
    console.log(`[${mid}] auto_spin → ${spinnerId}`);
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data) => {
    ...
    // Chat tarixi — yalnız bu socket-ə
    ...
    broadcastCounts();
    startIfReady(mid);
    console.log(`[${mid}] join uid=${uid}, total=${pl.length}`);
  });
  ...
  socket.on('spin', (data) => {
    ...
    // Timer-ı durdur
    if (ts.timer) { clearTimeout(ts.timer); ts.timer = null; }
    // Yalnız digərlərinə göndər
    socket.to(mid).emit('spin', data);
    console.log(`[${mid}] spin by=${data.by} winner=${data.winner}`);
  });
  ...
  socket.on('next_turn_ready', (data) => {
    ...
    ts.pending = setTimeout(() => {
      ...
      ts.current = null;
      nextTurn(mid);
      console.log(`[${mid}] next_turn_ready → nextTurn`);
    }, 500);
  });
  ...
  socket.on('your_turn',      ()  => {}); // server idarə edir, client-dan gələni yoksay
  ...
  socket.on('disconnect', () => {
    ...
    // Eyni uid yeni socket-lə qoşulubsa — köhnə disconnect-i yoksay
    ...
    broadcastCounts();
    console.log(`[${mid}] disconnect uid=${uid}, remaining=${pl.length}`);
  });
});
