...
function getMasaCounts() {
  ...
  MASA_ROOMS.forEach(id => {
    const sids = masalar.get(String(id));
    let m = 0, f = 0;
    if (sids) {
      sids.forEach(sid => {
        ...
        if (g === 'f' || g === 'female' || g === 'qiz' || g === 'k' || g === 'qadin') f++;
        else m++;
      });
    }
    counts[id] = { m, f, total: (sids ? sids.size : 0) };
  });
  return counts;
}
...
function sendPlayerList(masaId) {
  ...
  // After player list, broadcast updated counts to everyone
  broadcastMasaCounts();
}
