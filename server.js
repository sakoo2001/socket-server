...
io.on('connection', (socket) => {
  ...
  socket.on('spin', (data) => {
    ...
    const mid = String(data.masaId);
    const ts  = getTurn(mid);
    // Spinner doğru kişiyse timer'ı durdur
  ...
  socket.on('next_turn_ready', (data) => {
    ...
    // Çift next_turn_ready'yi önle — 600ms içinde sadece 1 kez işle
