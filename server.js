...
function nextTurn(mid) {
  ...
  console.log(`[${mid}] your_turn spinner=${spinnerId} target=${targetId} players=${pl.length}`);
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data) => {
    ...
    console.log(`[${mid}] join uid=${uid} name=${data.name} total=${roomPlayers(mid).length}`);
  ...
  socket.on('disconnect', (reason) => {
    ...
    if (room[uid] && room[uid].socketId !== socket.id) {
      console.log(`[${mid}] disconnect uid=${uid} IGNORED (new socket exists)`);
      return;
    }

    delete room[uid];
    console.log(`[${mid}] disconnect uid=${uid} reason=${reason} remaining=${roomPlayers(mid).length}`);
...
setInterval(() => {
  ...
  if (cleaned) console.log(`[cleanup] ${cleaned} empty rooms removed`);
}, 10 * 60 * 1000);

httpServer.listen(PORT, () => {
  console.log(`Socket.IO listening on port ${PORT}`);
  console.log(`Rooms: 0, Ready.`);
});
