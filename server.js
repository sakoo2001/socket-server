...
io.on('connection', (socket) => {
  ...
  // ── Profil bəyənmə — real-time, bütün istifadəçilərə yayılır ──
  socket.on('profile_like', (data) => {
    ...
    // 1) Hədəf istifadəçinin öz socketinə göndər (o da biləcək)
    const targetSids = findSocketsByUserId(payload.targetUserId);
    targetSids.forEach(sid => {
      if (sid !== socket.id) io.to(sid).emit('profile_like', payload);
    });

    // 2) Eyni masadakı hər kəsə yayıl — açıq profil pəncərəsi varsa yenilənsin
    if (payload.masaId) {
      socket.to(getMasaRoom(payload.masaId)).emit('profile_like', payload);
    } else {
      // masaId yoxdursa bütün serverə yay
      socket.broadcast.emit('profile_like', payload);
    }
