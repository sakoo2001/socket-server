...
/* ══ Bütün socketləri user ID-yə görə tap ══ */
...
/* ══ Balans yenilə + balance_update göndər ══ */
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data) => {
    ...
    users.set(socket.id, {
      ...
      dg:     parseInt(data.dg)     || 0,  // ← DG saxlanır
    });
    ...
    // Qoşulana öz cari balansını dərhal göndər (PHP-dən gəlir, amma double-check üçün)
  ...
  // ── Kalp satın al (buy_hearts) ──
  // Client optimistic update edir, server RAM-dəki dəyəri yeniləyib
  // balance_update geri göndərir (DB varsa ora da yazılmalı)
  socket.on('buy_hearts', (data) => {
    ...
    if (u.dg < price) {
      // DG çatmır — real dəyərləri geri göndər (client-i korrektə et)
    ...
    u.hearts += hearts;
    emitBalanceUpdate(u.userId, u.hearts, u.dg);
    console.log(`[buy_hearts] userId=${u.userId} hearts+${hearts} dg-${price} → hearts=${u.hearts} dg=${u.dg}`);
  });

  // ── Musiqi üçün kalp xərci (music_cost) ──
  socket.on('music_cost', (data) => {
    ...
    u.hearts -= cost;
    emitBalanceUpdate(u.userId, u.hearts, u.dg);
    console.log(`[music_cost] userId=${u.userId} hearts-${cost} → hearts=${u.hearts}`);
  });
