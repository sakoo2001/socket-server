...
function sendPlayerList(masaId) {
  ...
  // Həmişə hamıya yayıl — otaq boş olsa belə
  broadcastMasaCounts();
}
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data) => {
    ...
    // Odanın aktiv şişəsini yeni istifadəçiyə göndər
  ...
  socket.on('bottle_change', (data) => {
    const masaId = String(data.masaId || 1);
    const src = data.src || '';
    // Odanın şişəsini yaddaşda saxla
    if (src) activeBottle.set(masaId, src);
    // Odadakı DİGƏRLƏRİNƏ yayıl (özü artıq dəyişdi)
  ...
  // ── Çerçevə dəyişdirildi — real-time hər kəsə yayılır ──
  ...
  socket.on('disconnect', () => {
    ...
    if (u && u.masaId) {
      ...
      // users.delete BEFORE sendPlayerList — correct count
      users.delete(socket.id);
